...
export default function ScanResults({ scanData, onNewScan }: ScanResultsProps) {
  const { result, fileName, fileSize, sha256Hash, scanTimestamp, scanType, url } = scanData;
  const isURLScan = scanType === 'url';
  ...
  const downloadReport = () => {
    const reportText = isURLScan
      ? `
SafeScanX URL Security Analysis Report
======================================
...
URL Safety Check:
${result.urlCheckResult?.found
  ? `✓ URL found in threat database
   Detections: ${result.urlCheckResult.detections}
   Categories: ${result.urlCheckResult.categories?.join(', ') || 'None'}
   Verdict: ${result.urlCheckResult.verdict}`
  : '✗ URL not found in threat databases - Appears safe'}
...
2. Heuristic Analysis:
   ${result.heuristicResult?.flags?.length ?
     `Flags: ${result.heuristicResult.flags.join(', ')}` :
     'No heuristic flags detected'}

3. Static Analysis:
   ${result.staticAnalysisResult?.detectedPatterns?.length ?
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
        <div className="p-6 border-b border-gray-200">
          <h3 className="text-lg font-semibold text-gray-900 flex items-center">
            ...
            {isURLScan ? 'URL Information' : 'File Information'}
          </h3>
        </div>
              ...
              {result.urlCheckResult?.found ? (
                <div className="space-y-3">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-yellow-700 font-medium">URL found in threat database</span>
                  </div>
                  <div className="text-sm text-gray-600 space-y-2">
                    <p><strong>Detections:</strong> {result.urlCheckResult.detections}</p>
                    <p><strong>Verdict:</strong> <span className={getThreatColor(result.urlCheckResult.verdict)}>{result.urlCheckResult.verdict}</span></p>
                    {result.urlCheckResult.categories && result.urlCheckResult.categories.length > 0 && (
                      <div>
                        <strong>Categories:</strong>
                        <div className="flex flex-wrap gap-2 mt-1">
                          {result.urlCheckResult.categories.map((category, idx) => (
                    ...
                    {result.urlCheckResult.analysisStats && (
                      <div className="mt-3 pt-3 border-t">
                        <strong>Analysis Stats:</strong>
                        <div className="grid grid-cols-2 gap-2 mt-2 text-xs">
                          <p>Malicious: <span className="text-red-600 font-bold">{result.urlCheckResult.analysisStats.malicious}</span></p>
                          <p>Suspicious: <span className="text-yellow-600 font-bold">{result.urlCheckResult.analysisStats.suspicious}</span></p>
                          <p>Harmless: <span className="text-green-600 font-bold">{result.urlCheckResult.analysisStats.harmless}</span></p>
                          <p>Undetected: <span className="text-gray-600 font-bold">{result.urlCheckResult.analysisStats.undetected}</span></p>
                        </div>
                      </div>
                    )}
                  </div>
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-green-600 font-medium">URL not found in threat databases - Appears safe</span>
                </div>
              )}
            </div>
          </div>
                  ...
                  <div className="p-6">
                    <div className="space-y-3">
                      <div className="flex items-center space-x-3">
                        ...
                        <span className={`text-lg font-semibold capitalize ${getThreatColor(result.behavioralAnalysisResult?.verdict || 'unknown')}`}>
